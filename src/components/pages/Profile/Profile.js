...
import {
    Inventory, InventoryItems, InventoryItemsDiv, LastDiv, Lasttext, ProfileContainer, Seperater
} from "./ProfileStyles";
...
export const Profile = (props) => {
    ...
    return (
        <>
            {props.kiltWallet ? <>
                {!account && <StatusButton onClick={() => props.connectAccount()} style={{ width: 'auto', padding: '0 20px' }}>CONNECT METAMASK</StatusButton>}
                {account && <ProfileContainer>
                    <OverlayModal
                        {...props}
                        item={modalItem}
                        isOpen={toggle}
                    ...
                    <Inventory>
                        <InventoryItemsDiv>
                            {ownedItems.map((item, index) => (
                                <InventoryItems key={index} style={{ textAlign: 'center', padding: '2%' }} onClick={() => openOverlay(item)}>
                        ...
                        <TextSapn style={{ fontWeight: 700, fontSize: '14px' }}>INVENTORY</TextSapn>
                    </Inventory>
                    ...
                    <Inventory>
                        ...
                        <TextSapn style={{ fontWeight: 700, fontSize: '14px' }}>LISTED FOR SALE</TextSapn>
                    </Inventory>
                    <Lasttext>
                        <LastDiv><TextSapn>{parseFloat(tokenBalance).toFixed(2)}</TextSapn></LastDiv>
                        <LastDiv><TextSapn>kORE</TextSapn></LastDiv>
                        <Seperater></Seperater>
                        <LastDiv><TextSapn>{parseFloat(nativeBalance).toFixed(3)}</TextSapn></LastDiv>
                        <LastDiv><TextSapn>{process.env.REACT_APP_COIN}</TextSapn></LastDiv>
                    </Lasttext>
                </ProfileContainer>}
            </> : <>
                <StatusButton onClick={() => props.connectSporran()} style={{ width: 'auto', padding: '0 20px' }}>
                    LOGIN WITH &nbsp; <img src="/images/kilt-logo.png" style={{ width: 50, height: 20 }} alt="Kilt-Logo" />
                </StatusButton>
            </>}
        </>
    );
